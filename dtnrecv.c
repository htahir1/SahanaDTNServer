...
static int create_db()
{
    ...
    int rc = sqlite3_open("payloadDB.db", &db);
    ...
    char *sql = "CREATE TABLE caseBundles(CaseNumber INT, Name varchar(50), DateofBirth varchar(50), Sex varchar(10), MobilePhone varchar(15), Email varchar(30), Country varchar(50), StreetAddress varchar(50), Postcode varchar(50), DamageAssessment varchar(50), Insurance varchar(20), Status varchar(50), Comments varchar(50));"
                
                "CREATE TABLE incidentBundles(IncidentNumber INT, IncidentType varchar(50), Name varchar(50), Exercise varchar(5), ZeroHour varchar(5), Country varchar(25), StreetAddress varchar(50), Postcode varchar(10), Comments varchar(50));"

                "CREATE TABLE eventBundles(EventNumber INT, Name varchar(50), EventType varchar(50), Exercise varchar(5), StartDate varchar(10), EndDate varchar(10), Comments varchar(50));";
...
static int store_db(char *buffer, u_int length)
{
    //printf("%s\n syore db inside");
    FILE* file;
    int argc;
    ...
    file = fopen("SahanaInterface.py","r");
    PyRun_SimpleFile(file, "SahanaInterface.py");
    //Py_Finalize();
...
//------------------------------------------------------------------------------------
    ...
    char getbundles [BUFSIZE];
    strcpy (getbundles, buffer);
    ...
    if(strcmp(s, "create_dvrcase") == 0)
    {
        ...
        char *sql = "insert into caseBundles (CaseNumber, Name, DateofBirth, Sex, MobilePhone, Email, Country, StreetAddress, Postcode, DamageAssessment, Insurance, Status, Comments) values (?,?,?,?,?,?,?,?,?,?,?,?,?)";

        rc = sqlite3_prepare_v2(db, sql, -1, &res, 0);
        
        int caseId = atoi(hand[0]);

        if (rc == SQLITE_OK) 
        {
            sqlite3_bind_int(res, 1, caseId);
            sqlite3_bind_text(res, 2, hand[1], strlen(hand[1]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 3, hand[2], strlen(hand[2]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 4, hand[3], strlen(hand[3]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 5, hand[4], strlen(hand[4]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 6, hand[5], strlen(hand[5]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 7, hand[6], strlen(hand[6]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 8, hand[7], strlen(hand[7]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 9, hand[8], strlen(hand[8]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 10, hand[9], strlen(hand[9]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 11, hand[10], strlen(hand[10]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 12, hand[11], strlen(hand[11]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 13, hand[12], strlen(hand[12]), SQLITE_TRANSIENT);
        } 
    ...
    else if(strcmp(s, "create_incident") == 0)
    {
        printf("%s this is the INCIDENT bundle");
        char* hand[8];
        ...
        char *sql = "insert into incidentBundles (IncidentNumber, IncidentType, Name, Exercise, ZeroHour, Country, StreetAddress, Postcode, Comments) values (?,?,?,?,?,?,?,?,?)";

        rc = sqlite3_prepare_v2(db, sql, -1, &res, 0);

        if (rc == SQLITE_OK) 
        {
            sqlite3_bind_int(res, 1, incidentNum);
            sqlite3_bind_text(res, 2, hand[0], strlen(hand[0]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 3, hand[1], strlen(hand[1]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 4, hand[2], strlen(hand[2]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 5, hand[3], strlen(hand[3]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 6, hand[4], strlen(hand[4]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 7, hand[5], strlen(hand[5]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 8, hand[6], strlen(hand[6]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 9, hand[7], strlen(hand[7]), SQLITE_TRANSIENT);
    ...
    else if(strcmp(s, "create_event") == 0)
    {
        printf("%s this is the EVENT bundle");
        ...
        char *sql = "insert into eventBundles (EventNumber, Name, EventType, Exercise, StartDate, EndDate, Comments) values (?,?,?,?,?,?,?)";

        rc = sqlite3_prepare_v2(db, sql, -1, &res, 0);

        if (rc == SQLITE_OK) 
        {
            sqlite3_bind_int(res, 1, eventNum);
            sqlite3_bind_text(res, 2, hand[0], strlen(hand[0]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 3, hand[1], strlen(hand[1]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 4, hand[2], strlen(hand[2]), SQLITE_TRANSIENT);
            sqlite3_bind_text(res, 5, hand[3], strlen(hand[3]), SQLITE_TRANSIENT);
    ...
    {
        printf("%sAcknowledgement Received");
        return 1;
    }
*/
    return 0;
...
{
    printf("hello and welcome to dtn_recv!!!\n");
    //create_db();
    int i;
    u_int k;
    int ret;
    ...
    if (count == 0) {
        printf("looping forever to receive the DTN bundles\n");
    } else {
    ...
    for (i = 0; (count == 0) || (i < count); ++i) {
        ...
        store_db(payload.buf.buf_val, payload.buf.buf_len);
